...
@click.argument('entity_db_file', type=click.Path(exists=True))
@click.argument('dataset_dir', type=click.Path(exists=True, file_okay=False), default='.')
@click.option('--embedding', is_flag=True)
@click.option('--dev-probs-file', type=click.File(mode='w'))
@click.option('--test-probs-file', type=click.File(mode='w'))
